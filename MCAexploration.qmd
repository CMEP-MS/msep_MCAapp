...
8.  Groupings were applied for region/area. These need to be modified because splitting on names doesn't work perfectly; but for now groupings occurred as follows:  
    a.  If the `LocationName` was *GULF OF MEXICO*, the station was assigned *Gulf*.  
    b.  If the `LocationName` was *MISSISSIPPI SOUND*, the station was assigned *Sound*.  
    c.  If the `LocationName` had the word *BAY* in it, the station was assigned to *Bays*.  
    d.  All other stations were assigned to *Rivers/Lakes/Tributaries*.    
:::



# Exploration by Parameter  
...
for (param in params) {
  ...
  # render child doc
  res <- knitr::knit_child("_childParameter.qmd", 
                            envir = environment(), 
                            quiet = TRUE)
  cat(res, sep = "\n")
}
